...
class ImprovedAgent(object):
	def __init__(self, env, pos=None):
		super(ImprovedAgent, self).__init__()
	...
	def select(self, env, prey_pos=None, predator_pos=None):
		# if prey in neighbors, select it
		if prey_pos in env.vertices[self.pos].neighbors:
			return prey_pos

		# shortest distances from agent to prey and predator
		a_prey = shortest_dist(self.pos, prey_pos, env, {predator_pos})
		...
		n_predator = {}
		for n in neighbors:
			n_prey[n] = shortest_dist(n, prey_pos, env, {predator_pos})
		...
		# farther from predator, closer to prey
		conds.append(lambda n: n_predator[n] > a_predator and n_prey[n] < a_prey)
		# farther from predator, not closer to prey
		conds.append(lambda n: n_predator[n] > a_predator and n_prey[n] == a_prey)
		# not closer to predator, closer to prey
		conds.append(lambda n: n_predator[n] == a_predator and n_prey[n] < a_prey)
		# not closer to predator, not closer to prey
		conds.append(lambda n: n_predator[n] == a_predator and n_prey[n] == a_prey)
